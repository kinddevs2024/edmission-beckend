...
async function makeUniqueGeneratedStudentEmail(
  firstName: string,
  lastName: string,
...
export async function listSchools(params?: {
  ...
  const userMap = new Map(
    users.map((u: { _id: unknown; name?: string; email?: string }) => [
      String(u._id),
      u,
    ]),
  );
...
export async function listMyStudents(
  ...
  const userIds = profiles
    .map((p: { userId?: unknown }) => p.userId)
    .filter(Boolean);
  const users = await User.find({ _id: { $in: userIds } })
    .select("email name mustChangePassword temporaryPlainPassword")
    .lean();
  const userMap = new Map(
    users.map(
      (u: {
  ...
  const data = profiles.map((p: Record<string, unknown>) => {
    const u = userMap.get(String(p.userId)) as
      | {
          email?: string;
          name?: string;
          mustChangePassword?: boolean;
    ...
    return {
      ...
      temporaryPassword: u?.mustChangePassword
        ? String(u?.temporaryPlainPassword ?? "") || undefined
        : undefined,
    };
  });
...
async function assertCounsellorAccount(
  counsellorUserId: string,
): Promise<void> {
...
function buildCounsellorStudentPayload(
  userRaw: Record<string, unknown>,
  profileRaw: Record<string, unknown>,
): CounsellorExcelStudentPayload {
  ...
  return {
    ...
    preferredCountries: Array.isArray(profileRaw.preferredCountries)
      ? profileRaw.preferredCountries.map(String).filter(Boolean)
      : [],
    interestedFaculties: Array.isArray(profileRaw.interestedFaculties)
      ? profileRaw.interestedFaculties.map(String).filter(Boolean)
      : [],
  };
}

async function getCounsellorStudentRows(counsellorUserId: string): Promise<
  Array<{
    id: string;
    userId: string;
...
> {
  ...
  const users = await User.find({ _id: { $in: userIds }, role: "student" })
    ...
    .lean();
  const userMap = new Map(
    users.map((user: Record<string, unknown>) => [String(user._id), user]),
  );
    ...
    return [
      {
        ...
        temporaryPassword: (
          ...
          ? String(
              (user as { temporaryPlainPassword?: string })
...
export async function getCounsellorStudentsExcelExportBuffer(
  counsellorUserId: string,
): Promise<Buffer> {
  const XLSX = require("xlsx");
...
function parseCounsellorStudentsExcel(buffer: Buffer): {
  rows: ParsedCounsellorStudentExcelRow[];
  errors: Array<{ row: number; name: string; message: string }>;
} {
  const XLSX = require("xlsx");
  const wb = XLSX.read(buffer, { type: "buffer", cellDates: false });
  const sheetName =
    (wb.SheetNames || []).find((name: string) => /student/i.test(name)) ||
    wb.SheetNames?.[0];
  ...
  for (let index = 0; index < rawRows.length; index++) {
    ...
    const lastName =
      trimString(
        raw["Last name"] ?? raw.lastName ?? raw.Surname ?? raw.surname,
      ) ?? "";
    const name =
      trimString(raw.Name ?? raw.name) ??
      [firstName, lastName].filter(Boolean).join(" ");
    ...
    rows.push({
      ...
      body: {
        ...
        graduationYear: parseNumFromText(
          raw["Graduation year"] ?? raw.graduationYear,
        ),
        preferredCountries: splitList(
          raw["Preferred countries"] ?? raw.preferredCountries,
        ),
        interestedFaculties: splitList(
          raw["Interested faculties"] ?? raw.interestedFaculties,
        ),
      },
    });
  }
...
async function findCounsellorStudentForImport(
  counsellorUserId: string,
  row: ParsedCounsellorStudentExcelRow,
) {
...
export async function importCounsellorStudentsFromExcel(
  counsellorUserId: string,
  buffer: Buffer,
): Promise<{
  created: number;
    ...
    try {
      ...
      if (existingProfile) {
        ...
        if (duplicate) {
          throw new AppError(
            409,
            "Email already registered outside your students",
            ErrorCodes.CONFLICT,
          );
        }
        const tempPassword = generateTempPassword();
...
async function assertStudentBelongsToCounsellor(
  studentUserId: string,
  counsellorUserId: string,
) {
...
export async function generateTempPasswordForStudent(
  ...
  if (!user.mustChangePassword) {
    throw new AppError(
      400,
      "Student has already set their password. Use reset password if needed.",
...
export async function deleteMyStudent(
  counsellorUserId: string,
  studentUserId: string,
) {
  await assertStudentBelongsToCounsellor(studentUserId, counsellorUserId);
...
export async function requestToJoinSchool(
  studentUserId: string,
  counsellorUserId: string,
) {
  const user = await User.findById(studentUserId);
  ...
  const existing = await SchoolJoinRequest.findOne({
    studentId: studentUserId,
    counsellorUserId,
  });
  if (existing) {
    if (existing.status === "pending")
      throw new AppError(409, "Request already sent", ErrorCodes.CONFLICT);
    if (existing.status === "accepted")
      throw new AppError(409, "Already in this school", ErrorCodes.CONFLICT);
    // Rejected: allow new request
    await SchoolJoinRequest.updateOne(
      { _id: existing._id },
      { status: "pending" },
    );
  } else {
    await SchoolJoinRequest.create({
      studentId: studentUserId,
      counsellorUserId,
      status: "pending",
    });
  ...
  const schoolName =
    (profile as { schoolName?: string })?.schoolName ?? "School";
...
export async function listJoinRequests(
  ...
  const studentIds = list
    .map((r: { studentId?: unknown }) => r.studentId)
    .filter(Boolean);
  ...
  const profiles = await StudentProfile.find({ userId: { $in: studentIds } })
    ...
    .lean();
  const userMap = new Map(
    users.map((u: { _id: unknown; email?: string; name?: string }) => [
      String(u._id),
      u,
    ]),
  );
  const profileMap = new Map(
    profiles.map((p: Record<string, unknown>) => [String(p.userId), p]),
  );
  const data = list.map((r: Record<string, unknown>) => {
    const u = userMap.get(String(r.studentId));
    const p = profileMap.get(String(r.studentId)) as
    ...
    const nameFromProfile = p
      ...
      : "";
    const studentName =
      ((u as { name?: string })?.name ?? nameFromProfile) || "—";
    return {
      id: String(r._id),
...
export async function acceptJoinRequest(
  ...
  const req = await SchoolJoinRequest.findOne({
    _id: requestId,
    counsellorUserId,
  });
  if (!req) throw new AppError(404, "Request not found", ErrorCodes.NOT_FOUND);
  if (req.status !== "pending")
    throw new AppError(400, "Request already processed", ErrorCodes.VALIDATION);
...
async function getCounsellorStudentProfileContext(counsellorUserId: string) {
  ...
  const profiles = await StudentProfile.find({
    ...
    .lean();
  const userIds = profiles
    .map((profile: Record<string, unknown>) => profile.userId)
    .filter(Boolean);
  ...
  for (const profile of profiles as Record<string, unknown>[]) {
    const user = userMap.get(String(profile.userId)) as
...
export async function listMyApplications(
  ...
  const skip = (page - 1) * limit;
  const { profileIds, profileMap } =
    await getCounsellorStudentProfileContext(counsellorUserId);
  ...
  if (
    query.studentUserId &&
    mongoose.Types.ObjectId.isValid(query.studentUserId)
  ) {
  ...
  const profileItems = profileList.map((interest: Record<string, unknown>) => {
    ...
    const university = interest.universityId as
      | { _id?: unknown; universityName?: string }
      | undefined;
    return {
  ...
  const catalogItems = catalogList.map((interest: Record<string, unknown>) => {
    const studentProfileId = String(interest.studentId ?? "");
    const student = profileMap.get(studentProfileId);
    const university = interest.catalogUniversityId as
      | { _id?: unknown; universityName?: string }
      | undefined;
    const rawUniversityId =
      university?._id != null
        ? String(university._id)
        : String(interest.catalogUniversityId ?? "");
    return {
  ...
  const merged = [...profileItems, ...catalogItems].sort(
    (a, b) =>
      new Date((b.createdAt as Date | string | undefined) ?? 0).getTime() -
      new Date((a.createdAt as Date | string | undefined) ?? 0).getTime(),
  );
  return {
...
export async function listMyOffers(
  ...
  if (query.type === "offer" || query.type === "scholarship")
  ...
  const data = documents.map((document: Record<string, unknown>) => {
    ...
    const student = profileMap.get(studentProfileId);
    const university = document.universityId as
      | {
          _id?: unknown;
...
export async function searchStudentsForInvite(
  ...
  const myStudentIds = new Set(
    myProfileStudentIds.map((p: { userId?: unknown }) => String(p.userId)),
  );
  ...
  const pendingIds = new Set(
    pendingInvitationStudentIds.map((p: { studentUserId?: unknown }) =>
      String(p.studentUserId),
    ),
  );
  const data = studentUsers
    .filter(
      (u: { _id: unknown }) =>
        !myStudentIds.has(String(u._id)) && !pendingIds.has(String(u._id)),
    )
    .slice(0, limit)
...
export async function inviteStudentToSchool(
  counsellorUserId: string,
  studentUserId: string,
) {
  const user = await User.findById(counsellorUserId);
  ...
  const existingLink = await StudentProfile.findOne({
  ...
  let invitation = await SchoolInvitation.findOne({
    counsellorUserId,
    studentUserId,
  });
  if (invitation) {
    ...
    if (invitation.status === "declined") {
      await SchoolInvitation.findByIdAndUpdate(invitation._id, {
        status: "pending",
  ...
  const schoolName =
    (profile as { schoolName?: string })?.schoolName ?? "A school";
  ...
  return {
    success: true,
    message: "Invitation sent. The student can accept or decline.",
  };
}
...
export async function listMyInvitations(
  ...
  const studentIds = list
    .map((r: Record<string, unknown>) => r.studentUserId)
    .filter(Boolean);
  const users = await User.find({ _id: { $in: studentIds } })
    .select("email name")
    .lean();
  const userMap = new Map(
    users.map((u: Record<string, unknown>) => [String(u._id), u]),
  );
  const data = list.map((r: Record<string, unknown>) => {
    const u = userMap.get(String(r.studentUserId)) as
      | { email?: string; name?: string }
      | undefined;
    return {
      id: String(r._id),
...
export async function cancelSchoolInvitation(
  ...
  const invitation = await SchoolInvitation.findOne({
    _id: invitationId,
    counsellorUserId,
  });
  if (!invitation) {
...
export async function acceptSchoolInvitation(
  ...
  await SchoolInvitation.findByIdAndUpdate(invitationId, {
    status: "accepted",
  ...
  const schoolName =
    (counsellorProfile as { schoolName?: string })?.schoolName ?? "School";
  const studentUser = await User.findById(studentUserId)
    .select("name email")
    .lean();
  const studentName =
    (studentUser as { name?: string; email?: string })?.name ??
    (studentUser as { email?: string })?.email ??
    "A student";
  await notificationService.createNotification(counsellorUserId, {
...
export async function listSchoolInvitationsForStudent(studentUserId: string) {
  const list = await SchoolInvitation.find({ studentUserId, status: "pending" })
    .sort({ createdAt: -1 })
    .lean();
  const counsellorIds = list
    .map((r: Record<string, unknown>) => r.counsellorUserId)
    .filter(Boolean);
  const profiles = await CounsellorProfile.find({
  ...
  return list.map((r: Record<string, unknown>) => {
    const profile = profileMap.get(String(r.counsellorUserId)) as
      | { schoolName?: string; city?: string; country?: string }
      | undefined;
    return {
      id: String(r._id),
...
export async function deleteDocumentForStudent(
  counsellorUserId: string,
  studentUserId: string,
  documentId: string,
) {
